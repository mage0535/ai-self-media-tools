...
- Bilibili recovery requires a fresh interactive login if `cookies/bilibili_main.json` is absent:

```bash
cd "$SOCIAL_AUTO_UPLOAD_HOME"
./venv/bin/python sau_cli.py bilibili login --account main
./venv/bin/python sau_cli.py bilibili check --account main
```
...
- Server `social-auto-upload` checks:
  - Douyin `main`: `valid`
  - Bilibili `main`: `invalid` until `cookies/bilibili_main.json` is created by interactive login; fallback backends are configured.
- Added tests for:
...
- Official GitHub repository: `mage0535/ai-self-media-tools`
...
- clean source bundle exported to `/root/e2e-content-platform/src`
- clean install root created at `/root/e2e-content-platform/home`
...
1. Keep the clean packaged project synced to the official GitHub repository `mage0535/ai-self-media-tools`.
...
- GitHub push blocked by credential rotation (`gh` logged in as `hermes0535`, `mage0535` token expired). Push pending token refresh.
