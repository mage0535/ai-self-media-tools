...
- Douyin auth refresh must prefer the CN-host noVNC real-Chrome profile path when normal Hermes QR login is unstable. Headless QR generation, static QR screenshots, or local multi-hop proxy Chrome sessions are not sufficient success evidence by themselves; the workflow must export storage state from the logged-in CN-host profile and keep `/root/social-auto-upload/cookies/douyin_uploader/main.json` as the canonical account file.
