...
- v8b direct-shell configuration omitted `HERMES_HOME`/the scripts directory. Read-only gateway inspection found `HERMES_HOME` configured and diagram scripts present. The direct-run failure is not proof the gateway lacks those modules.
- v8c with the Hermes scripts path failed the eight-asset gate at 6/8. After `88253a8`, a Linux semantic recovery smoke on its script returned 8 Pexels assets, 8 distinct IDs and 8 distinct hashes; no generated fallback was accepted. Local full pytest: 1902 passed plus 37 subtests.
- v8d passed content and generated eight slide backgrounds, eight Hojo TTS segments and encoded scene segments before BGM. Its new Audio Library track was registered during download and immediately rejected by a second fingerprint checker as a duplicate. `fd2a9e8` Linux focused tests passed 4/4. Local full pytest: 1903 passed plus 37 subtests; project/privacy 594/0, license 66/0.
- A direct retained-run probe could not reconstruct missing visual-assets input, modified v8d intermediate files and failed closed; it is explicitly not a successful revalidation. A fresh v8e run was interrupted after detecting a concurrent user/Hermes Kuaishou render. The user render was not stopped or modified.
- The production optimization handoff at `$SERVER_PHASE0_WORKTREE/docs/continuous-development/2026-09-13-production-optimization-handoff.md` describes dirty files on `phase0/integration-baseline@79e7812`. Its platform DNA covers 19 rulebook channels and its own old-tree test run reports 943 passed, 33 subtests. No such DNA was merged into the current branch. Read-only 10:13 CST check found isolated staging at `fd2a9e8`, production symlink still at signed `2f4f612`, and the Hermes Kuaishou render no longer running. This is a point-in-time observation, not deployment acceptance.

## 2026-09-13 YouTube V8F Artifact And Manual Review

- Isolated v8f produced an MP4 in 511.9 seconds. Pipeline and independent Task9 probes passed; job state `review_required`. BGM provider `youtube_audio_library`, source `Acoustic Meditation 2`, YouTube-use scope with attribution still requiring Studio review; its audio was mixed to stereo. ASR similarity 0.9549 and terminal word coverage 0.9412. These are one-platform machine measurements, not publication or account-performance evidence.
- Contact-sheet and 1920x1080 cover inspection rejected the package. Cover text ended mid-word at `(Be`, comparison frames were empty; Pexels alt identified DeepSeek or ChatGPT imagery in a Claude tutorial; generic card labels and midword subtitle cues remained visible. The package must not be externally delivered.
- `fa14637` is a TDD correction for those negative examples. Focused cover/asset/subtitle/video tests passed 51; local full pytest returned 1910 passed plus 37 subtests in 356.27 seconds. Project/privacy audit scanned 594 files with zero issues; license audit checked 66 capabilities with zero issues. Linux staging and a fresh end-to-end run still remain after this commit.

## 2026-09-13 YouTube V8G And Expanded Asset Smoke

- v8g ended failed after 379.0 seconds with `visual_assets_incomplete:6/8`. It did not produce or deliver a video. The generator's long attempt timed out at 180 seconds and its compact retry succeeded; this is recovery evidence, not an efficiency pass.
...
## 2026-09-13 YouTube V8I-V8J Isolated Failure Evidence

- Staging advanced cleanly to `505d3e5` and passed 106 focused tests, privacy 593/0 and license 66/0. The signed production symlink still pointed to `production-runtime-v8-2f4f612-20260906`; the project timer was inactive.
- v8i was interrupted because the mutable Hermes runtime began a competing Kuaishou render. Only the isolated v8i session was stopped; no mutable production process, database or work artifact was changed.
- v8j isolated root is recorded by the private server under the `youtube-v8j-20260913-125555` Canary alias; job `6a39c234218c4a03`. `case-result.json` records 532.8 seconds, `job_state=failed`, `pipeline_passed=false`, `probe_passed=false`. Runner manifest says `status=cover_failed`, `error=cover background failed platform/text conflict gate`. Its intermediate MP4 is 66.811 seconds at 1280x720 with 44.1kHz stereo audio, but ASR, accepted cover and handoff are absent.
- `video_toolchain_runner_manifest.json` shows three source URLs explicitly named ChatGPT among eight backgrounds for a Claude tutorial. The cover selector's zero-score rule rejected every background despite at least one source describing a person writing a project brief on a laptop. The horizontal model input also contained the vertical YouTube Shorts 1080x1920 rule.
- Local regression for the correction: `python -m pytest -q --junitxml=artifacts/test-reports/p10-skill-format-cover-brand.xml` returned 1918 passed plus 37 subtests in 377.93 seconds; the final focused suite returned 93 passed. `python -m content_platform project-audit` returned 594 files and no issues; `python scripts/license_audit.py` returned 66 capabilities and no issues. No fresh Linux Canary has yet validated this correction.

## 2026-09-13 V8K-V8L Isolation And Live Prompt Probe

- Clean staging `f970b82` passed 93 related tests, project/privacy audit 593/0 and license audit 66/0. Signed production symlink and inactive timer were unchanged.
- v8k's first preflight used an incorrect hotspot root and created no job. Its second attempt created job `f573f6624f3e483f` but was rejected before model invocation because the harness enabled a special canary session without verified model selectors. Neither attempt is generation evidence.
- v8l used a fresh isolated root with Hermes's active-model path and job `e35e4725a4fa4630`. The live model command contained `horizontal_video` and did not contain the Shorts dimension or `YouTube Shorts`, demonstrating that the new format filter reached model input.
- v8l ended `failed` after 150.9 seconds; the only pre-render failure was `card_6_narration_display_duplicate`. Saved cards show the headline and supporting label copying the sixth spoken beat. No TTS, MP4 or handoff was produced. The local regression first reproduced the gate failure and then passed after the content-specific summary correction. Full `artifacts/test-reports/p10-video-card-refinement.xml` returned 1920 passed plus 37 subtests in 384.43 seconds; privacy 594/0, license 66/0. A fresh Linux Canary remains pending.

## 2026-09-13 V8M And Mutable Optimization Review

- v8m job `c47ef02a7e71419f` ran at staging `883df90` with verified same-platform input and Hermes's active-model path. It ended `failed` after 648.1 seconds with `card_5_narration_display_duplicate`, `card_6_narration_display_duplicate` and `card_7_narration_display_duplicate`. Saved cards show contiguous title/support slices reconstructing each TTS beat. No accepted video or handoff exists.
- A local eight-card regression reproduced those exact failures. Content-specific summaries now replace the copied fragments while the pre-render gate remains fail-closed. Focused verification passed after the change; full regression is pending.
- Read-only server review found the new optimization status only in the mutable checkout. That checkout remains on its older main commit with 44 modified and 33 untracked entries. The listed prompt/source/cache modules are untracked prototypes and are not staging or signed-production evidence.
- The current branch already has stronger twelve-platform content-form routing, rule consumption hashes, provider parent/child telemetry, image intent routing and artifact/effect gates. The prototype's useful design is incorporated in `docs/CONTENT_GEN_ENHANCEMENT.md` and the Sol plan; no dirty source file was copied. Production symlink and timer were not changed.

## 2026-09-14 Image Workflow Server Review

- Read-only server status: mutable Hermes main remains on the older `6f4c88a` commit with many modified/untracked files; the signed production symlink remains at `production-runtime-v8-2f4f612-20260906`. The new image-provider file is larger and recently modified, but that does not mean the signed production release loads it.
- Relative to its old commit, the mutable checkout changed image provider routing, SenseNova/Pixazo calls, stock retouch, a prompt-bundle CLI, media-quality gates and documentation. Its stated 967 passed plus 33 subtests is specific to that checkout. Current branch already implements the major provider/intent capabilities and stronger media evidence.
- Code review found unsafe deltas not migrated: `no text` triggers retouch, an edit branch swallows exceptions, quality-first may insert OpenAI regardless of paid authorization, and a lane gate bans functional cat/dog roles from AI knowledge videos outside the pet account. The document also lists Wikimedia, Playwright MCP and Apify as non-executable plans.
- New current-branch TDD: four image-provider red tests, one MediaBridge paid-routing red test and one retouch-failure red test reproduced real policy/integrity gaps. After targeted fixes, the related suite returned 211 passed. Final full regression returned 1932 passed plus 37 subtests in 362.99 seconds; project/privacy 594/0, license 66/0, compileall and diff check passed. Linux staging and new isolated media Canary remain; no production or publisher was modified.

## 2026-09-14 Optional Mascot Policy Evidence

- Static review confirmed the current branch previously assigned cat and dog roles to every AI-lane blueprint and marked the global AI-knowledge mascot policy as enforced. That was a real project defect independent of the mutable server's opposite blanket-ban gate.
- Three red tests covered generic AI/no mascot, topic-matched AI/mascot and explicit disable. After implementation, the blueprint/generator/pre-render set returned 38 passed; the rulebook assertion raises the focused total to 39. The final full suite is 1932 passed plus 37 subtests.
- The accepted state is conditional selection plus functional validation, not mandatory injection or global rejection. No server checkout, production release, timer or publisher was changed by this correction.

## 2026-09-14 Isolated Image Provider Smoke And Manual Review

- Clean staging `0dc6ad8` returned 181 related tests, project/privacy 593/0, license 66/0 and compileall PASS. The isolated image smoke used the mutable runtime only as a private credential source; its output and cache were under a temporary test root, paid providers and Agnes automatic selection were disabled, and no production job or publisher was invoked.
- Transport results: Stock selected Pexels in 0.87 seconds; SenseNova U1.5 Lite returned 5.8MB in 22.81 seconds; Cloudflare FLUX returned 274KB in 1.59 seconds; Pollinations returned 34KB in 0.66 seconds; auto reused the SenseNova cache in 0.03 seconds. These timings are one run, not provider guarantees.
- Independent `visual_gate.py` passed auto, Cloudflare, Pollinations and SenseNova but rejected Stock for small/low-variance output. Manual contact-sheet review found Stock and Pollinations poorly matched to automated content production and visible branding on the SenseNova/auto image. This disproves the old smoke's aggregate `ok=true` as production evidence.
...
- `46440b0` was pushed and isolated Linux staging advanced cleanly. Linux focused regression returned 224 passed plus 8 subtests in 22.69 seconds; project/privacy audit scanned 594 files with zero issues; license audit checked 66 capabilities with zero issues; compileall passed. Signed production remained `2f4f612`; no timer, MCP target, publisher, mutable checkout or shared database changed.
