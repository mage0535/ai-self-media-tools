...
- Retain the dedicated WeChat metrics timer in release inventory; its definition is not authorization to enable it. Config rollback must complete before any old service is restarted (ordering hardening remains pending).
