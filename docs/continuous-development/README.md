...
- The Hojo TTS milestone replaces renderer-specific provider selection with one project-owned runtime. An approved durable quality gate enables Hojo-first for Chinese and English, every output is normalized and probed at 44.1kHz stereo, Edge remains the recorded fallback, and Kokoro is inventory-only. VoiceEngine, film, Kuaishou and landscape render paths are covered; production activation is still pending the signed P10 release process.
- X/Twitter same-lane collection now has strict staging acceptance: the default direct queries returned seven current status cards and all seven passed identity, time, named-metric, lane and snapshot contracts. This is collector evidence only, not X delivery proof.
- TikTok now has a project-owned official Creative Center Top Videos adapter for the public `Technology & Finance` preview. It remains a distinct official-reference layer; the current TikTok search failure is preserved and the four-item industry preview cannot claim AI same-lane Top3 readiness.
- Authentication state is now purpose-aware for Kuaishou: creator-center cookies retain official inspiration access but no longer trigger doomed public-work searches. Video Channels remains explicitly auth-required from its saved login-page evidence.
- Regional fallback now probes its endpoint before use and preserves direct-route evidence when the proxy is down. Xiaohongshu currently returns its official IP-risk page directly while the disabled CN tunnel is unavailable; this remains a visible collector blocker, not a generic failure.
- Current Douyin AI/pet collection remains exact-account auth-required, and WeChat's 20 Sogou articles remain non-metric references. These explicit partial states prevent weak models from converting available-but-insufficient data into false same-lane Top3 claims.
- The next acceptance change is Canary selection-mode support: strict verified-hotspot evidence and strict audited editorial fallback are separate valid inputs. The latter must keep associated hotspot empty and cannot receive hotspot scoring.
- That selection-mode contract is now implemented locally: strategy hash, current plan date, three empty recaptures and seven-day dedupe are required, and ambiguous hotspot/editorial inputs fail closed. Linux contract verification is next.
- The first isolated YouTube content Canary reached deterministic text hygiene and was correctly blocked because its final standalone CTA was missing punctuation. The local follow-up completes only an exact allowlist of known CTA lines before the unchanged hygiene gate; arbitrary truncated prose remains blocked. Full regression is `1874 passed` plus `37 subtests`; Linux staging and a fresh YouTube Canary remain next.
- The fresh YouTube retry then exposed a distinct Hermes transport boundary: direct OpenAI Codex OAuth inference returned an HTML edge 403 while the same active identity succeeded through the configured US fallback. The generator now classifies that exact edge response separately and permits one proxy retry; credential rejection remains fail-fast. Full regression is `1875 passed` plus `37 subtests`; a new Linux run is next.
- The next isolated YouTube run completed real generation, eight scene assets, Hojo TTS, online BGM, subtitles, final video, cover, ASR and manual handoff isolation. Independent probing passed every media dimension but rejected `shotcraft_moves` because it was only output-verified. The local correction binds Shotcraft scene/move mappings to the final MP4 hash and named frame-difference probe; full regression is `1877 passed` plus `37 subtests`. Linux re-probe remains next.
- The same run exposed a legacy archive helper that assumed every package lived under the repository in `platform/date/render`. The helper now supports either that legacy layout or a direct external package root and records only package-relative source labels. Full regression is `1879 passed` plus `37 subtests`; Linux evidence-only revalidation remains next.
- Retained-artifact revalidation exposed that an old checkpoint preserved Shotcraft's former `output_verified` policy even after the registry became stricter. Resume now preserves capability selection but applies the current registry verification level. Full regression is `1880 passed` plus `37 subtests`; final Linux re-probe remains next.
- Retained re-probe then passed every prior machine check, but manual review and full ASR proved the horizontal YouTube video had been truncated from about 68.8 to 59.8 seconds by a Shorts-only rule. Horizontal YouTube is now exempt and ASR must independently cover the final narration sentence. Full regression is `1882 passed` plus `37 subtests`; the old MP4 remains a rejected negative sample.
- Linux staging at `a3e6c02` confirmed the new gate rejects that retained MP4 for missing terminal narration despite its passing whole-script similarity. The next YouTube work is cover/scene/copy quality hardening followed by a fresh full-length Canary; production and timers remain unchanged.
