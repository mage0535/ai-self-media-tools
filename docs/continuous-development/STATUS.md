...
|---|---|---|---|---|
| Sol E Canary topic-evidence modes | Codex primary | server staging evidence and four coordination documents | verified | preserve hotspot/editorial exclusivity; use current evidence to start serial artifact Canaries |
| Sol E YouTube real artifact Canary | Codex primary | Task9 evidence identity, isolated server inputs/artifacts and four coordination documents | identity_fix_local | commit non-native identity preservation; repeat preflight before any horizontal-video generation |
...
## 2026-09-13 YouTube V7-V8D Recovery And Platform DNA Review

- Owner: Codex. Shared files now reserved: these four coordination documents. No production checkout, service, gateway, account or timer change is authorized by this work item.
- Fresh strict YouTube same-lane collector returned 24 items; the private Task9 input for 2026-09-13 retained `same_lane_hot_work`, `native_verified=false`. The collector unexpectedly wrote a cache in the shared data root despite isolated output; cache-root isolation remains open.
- v7 failed after 929.2 seconds because the landscape renderer ignored card-bound asset paths from reselection and looked for sequential `bg_07/bg_08`. `b912d45` binds slides to verified card assets first. Full local regression: 1897 passed plus 37 subtests.
- v8b's direct shell lacked Hermes scripts path, unlike the active gateway; v8c with that path failed closed at 6/8 unique scene assets. `88253a8` selects landscape stock for YouTube/Bilibili and revisits successful semantic queries only after the diverse first passes. Linux smoke obtained 8 distinct IDs and 8 distinct SHA-256 values. Full regression: 1902 passed plus 37 subtests.
- v8d reached eight scene slides, Hojo TTS and segment encoding, but BGM download registered its own fingerprint before the subsequent uniqueness check. `fd2a9e8` removes download-time registration and supplies one locked, seven-day, cross-work check with same-work idempotence for both renderers. Full regression: 1903 passed plus 37 subtests; privacy 594/0; license 66/0.
- Direct revalidation of v8d was invalid: the original runner did not persist a complete `video_visual_assets.json`, and the bare runner touched intermediates before failing closed. Do not count it as acceptance. v8e was interrupted by Codex when a separate user/Hermes Kuaishou render was detected; that user process was not stopped.
- Server handoff `$SERVER_PHASE0_WORKTREE/docs/continuous-development/2026-09-13-production-optimization-handoff.md` was read. Its uncommitted platform-DNA prototype on `phase0/integration-baseline@79e7812` is not part of `fd2a9e8` or the signed production release. Its 943 passed plus 33 subtests is an old-tree result, not the current baseline.
- Next: after resource isolation, run fresh YouTube end-to-end Pipeline/probe/manual review; then implement DNA v2 by account and content form with real pre-generation consumption and deterministic gates. Do not enable timers, call a partial media run a successful Canary, or merge the dirty old branch.

## 2026-09-13 YouTube V8F Machine Pass, Manual Rejection

- Fresh v8f used isolated staging `dee1e83` with a same-day strict YouTube work record. It finished real Pipeline and Task9 independent probes in 511.9 seconds. Job state is `review_required`, no external publisher called; both machine checks passed without listed failures.
- Eight scenes, final MP4, online YouTube Audio Library BGM, history check, stereo mix, Hojo TTS, burned subtitles, ASR terminal coverage and local handoff were produced. This closes prior scene/BGM execution blockers for one YouTube case only.
...
## 2026-09-13 YouTube V8I-V8J Rule And Cover Failure

- `505d3e5` was pushed and clean staging passed 106 focused tests plus privacy/license audits. Isolated v8i was interrupted after a concurrent user/Hermes Kuaishou render started; Codex stopped only its own isolated process and preserved the partial state.
- Fresh isolated v8j used the then-current staging code. It generated script, eight assets, Hojo TTS, BGM, subtitles and a 66.81-second horizontal MP4, but the runner failed after 532.8 seconds at `cover background failed platform/text conflict gate`. The Pipeline state is `failed`; there is no accepted ASR, cover package or handoff. MP4 existence is not completion.
- v8j exposed two separate quality faults: a horizontal YouTube model input included a 1080x1920 Shorts rule, and three accepted stock source URLs named ChatGPT while the tutorial was about Claude. The cover selector also gave zero relevance to a human writing a project brief at a laptop despite topic fit. The upstream error was hidden because the runner wrote its cover error only to JSON, not stderr.
- Local TDD now filters compiled skill rules by content form before the model input, rejects unrequested named-product assets using source URL/caption evidence, scores topic-related work scenes for cover selection, and emits the precise cover failure to stderr. Both new negative examples failed before implementation. Full local regression: 1918 passed plus 37 subtests; final related tests: 93 passed. Privacy 594/0 and license 66/0.
- Next: commit/push, advance clean Linux staging, rerun focused tests/audits, then run a fresh isolated horizontal YouTube Canary with the new rules and full manual cover/contact-sheet/ASR review. Do not reuse v8j as positive acceptance. Production symlink and overnight timer remain unchanged.

## 2026-09-13 YouTube V8K-V8L Canary Harness And Card Rejection

- `f970b82` was pushed and clean Linux staging passed 93 related tests, privacy 593/0 and license 66/0. v8k first failed before job creation because its test harness supplied `_inputs` instead of the parent root. Correcting that path created a job, but an unnecessary `HERMES_CANARY_SESSION` flag then required unavailable verified provider/model selectors. These are isolated harness failures, not production results.
- v8l used the same verified YouTube input with Hermes's active-model path. A live process probe confirmed `horizontal_video` in the model prompt and no `1080x1920` or `YouTube Shorts` markers. The Pipeline nevertheless failed after 150.9 seconds at `card_6_narration_display_duplicate`, before TTS, video or handoff.
- Its sixth beat was `Refine the weakest part. Caution: role-play cannot repair missing information.` The card headline copied `Refine the weakest part. Caution`; the supporting text copied the role-play clause. A new regression reproduced the pre-render failure. The local correction creates two content-grounded, nonverbatim summaries while preserving the unchanged anti-duplication gate.
- Full local regression completed: 1920 passed plus 37 subtests in 384.43 seconds; project/privacy audit 594/0, license audit 66/0 and diff check passed. Next: commit/push, verify clean Linux staging, then run a fresh isolated YouTube end-to-end Canary. v8j and v8l remain rejected negative samples; no publication and no timer change.

## 2026-09-13 YouTube V8M And Production Optimization Intake

- v8m used clean staging `883df90`, verified YouTube evidence and Hermes's active-model route. It passed content generation and content gates but failed after 648.1 seconds at pre-render with cards 5-7 reconstructing their spoken narration from a copied title plus copied support fragment. It created no accepted TTS/video/handoff package.
- A new full eight-card regression reproduces the observed v8m copy. The deterministic compiler now summarizes vague-input, generic-answer and staged-workflow beats instead of slicing contiguous narration. The anti-duplication gate remains unchanged.
- The mutable Hermes checkout's `CONTENT_GEN_ENHANCEMENT` update was reviewed read-only. Prompt Registry/Compiler, SearchSpec, source router and typed cache are useful design inputs, but their files are untracked in a 77-entry dirty checkout, cover fewer than all target profiles and encode YouTube as Shorts-only. They will not be copied or cherry-picked.
- The accepted integration keeps one authority: current content profile, compiled skill rules, bounded model input, capability registry, execution DAG and artifact/effect gates. Pexels/Pixabay are being aligned as parent-executed media providers for article, carousel and both video formats. Wikimedia, Playwright MCP and Apify stay pending until real adapters and receipts exist.
- Next: finish focused/full regression and audits, commit/push, verify staging, then run a fresh isolated YouTube Canary. Production and timers remain unchanged.

## 2026-09-14 Mutable Image Workflow Intake

- Read-only server check found the newly optimized image workflow in the dirty Hermes main checkout, not in the signed production release or this branch. The checkout's image provider diff adds SenseNova/Pixazo and auto-retouch, but the current branch already has content-intent routing, both provider integrations, atomic edit replacement and semantic/provenance gates. Do not copy the old provider file.
- Local red tests exposed four current-branch gaps: Pixazo could be selected by `auto` without paid opt-in; identical input-image bytes at different paths produced different cache keys; modified cache bytes were accepted; mixed AI/cat-dog prompts could select generic AI stock imagery. TDD fixes now pass.
- A fifth red test found the independent MediaBridge quality-recovery chain could also select Pixazo without paid opt-in; it is now guarded. An explicit retouch failure no longer returns the unedited stock photo as a successful result. Related image/media/video/capability suite: 211 passed. Full regression and fresh Linux validation remain pending.
- Server's `no text` edit trigger, silent edit exception handling, paid quality-first bypass and blanket ban on cat/dog visuals in non-pet AI lanes were rejected because they conflict with existing content and quality rules. Other server source adapters remain plan-only; no Wikimedia, Playwright MCP or Apify execution claim was promoted.
- Production symlink and timer were not changed. Final local regression returned 1932 passed plus 37 subtests in 362.99 seconds; privacy 594/0, license 66/0, compileall and diff check passed. Next: commit/push, clean staging verification and a fresh one-platform Canary. A successful image provider call is not publication or performance proof.

## 2026-09-14 Optional Mascot Policy Correction

- User clarified that cat/dog characters are not a mandatory hard rule for AI knowledge content. The prior `build_content_blueprint()` assigned both roles to every detected AI topic, and the channel rulebook declared the policy enforced across all AI knowledge formats.
