        ...
        <div class="shell">
          ...
          <div class="panel card" style="margin-top:18px;">
            ...
            <form id="binding-form" class="form-grid" onsubmit="saveBinding(event, '${{data.platform.key}}')">
              ...
              <div class="field"><label>赛道</label><input name="track" placeholder="例如 AI 自动化 / 科技测评" /></div>
              <div class="field"><label>现状</label><input name="current_status" placeholder="例如 起号中 / 稳定更新 / 商业化中" /></div>
          ...
          <div class="split" style="margin-top:18px;">
            <div class="panel card">
              <h3>账号分析</h3>
              ${{(data.account_analysis || []).map(row => `<div style='padding:12px 0;border-bottom:1px solid var(--line);'>
                <strong>${{escapeHtml(row.display_name)}}</strong>
                <div class='muted'>${{escapeHtml(row.account_key)}}${{row.track ? ' · 赛道：' + escapeHtml(row.track) : ''}}${{row.current_status ? ' · 现状：' + escapeHtml(row.current_status) : ''}}</div>
                <div class='muted'>${{escapeHtml(row.diagnosis)}}</div>
              </div>`).join('') || '<div class="muted">暂无账号分析</div>'}
            </div>
            <div class="panel card">
              <h3>LLM 详细建议</h3>
