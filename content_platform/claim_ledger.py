...
def build_grounded_technical_article(topic: str, ledger: list[dict[str, Any]] | None) -> dict[str, str]:
    ...
    blocks.extend([
        "## 把事实转成任务说明",
        "建议先用一句话写明任务边界：谁在什么场景下遇到了什么问题，最终需要得到什么结果。不要一开始就堆工具名称，也不要把想象中的功能写成既定事实。",
        "再把输入、处理动作和输出分别列开。输入写清已有资料和不能公开的内容；处理动作只描述希望完成的步骤；输出写清格式、用途和验收方式。这样整理后，事实与操作建议不会混在一起。",
        "如果某个步骤必须依赖外部资料，建议在任务说明里保留来源链接和核对日期。如果只是个人偏好，可以直接标为建议，不必包装成规范。",
        "## 规划文件与资源",
        "建议从最小可用范围开始，只准备当前任务确实需要的正文和资源。先完成一条边界清楚的小任务，再决定是否增加脚本、参考资料或图片等内容。",
        "可以给每份资源写一行用途说明，例如它服务于哪一步、由谁读取、输出会出现在哪里。不要仅凭文件已经放进目录，就判断它一定会被使用。",
        "检查文件时，可以逐项回答：名称是否清楚、内容是否与任务一致、引用是否仍然有效、敏感信息是否已经移除、下游是否真的能够读取。无法回答的项目先留在待验证区。",
        "## 设计可复核的执行过程",
        "建议把执行过程拆成准备、运行和验收三个部分。准备阶段核对输入与事实来源；运行阶段记录实际读取了什么；验收阶段检查输出是否满足任务说明。",
        "如果任务没有使用某份可选资源，不妨记录未使用原因。这样既能避免为了展示能力而强行调用无关内容，也能在下一次调整时知道缺口在哪里。",
        "例如需要补充说明时，可以先引用已经验证的原句，再写自己的理解，并明确标注后者属于解释。不要补写来源中没有出现的命令、兼容范围、性能数字或效果承诺。",
        "## 用检查清单完成复核",
        "建议按下面的顺序检查自己的结果：",
        "- 先确认标题回答的是当前问题，没有借用无关热门话题。",
        "- 再确认正文中的事实都能回到公开来源，来源标题和链接能够对应。",
        "- 检查建议是否使用了明确的建议语气，避免让读者把经验判断误认为官方规范。",
        "- 核对文件、字段和术语是否前后一致，避免缩写被截断或中英文名称被拆开。",
        "- 复查清单是否真的能执行，每一项都应有明确动作和可观察结果。",
        "- 最后删除没有证据的案例、数字、安装方式和效果描述，把暂时无法核对的内容标为待验证。",
        "完成这些检查后，可以让另一位读者只根据正文复述任务边界、事实依据和下一步动作。如果对方需要猜测，建议回到对应段落补清上下文，而不是继续增加新术语。",
