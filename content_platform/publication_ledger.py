...
class PublicationLedger:
    ...
    def _init(self):
        with self._connect() as conn:
            ...
            CREATE TABLE IF NOT EXISTS metric_windows (
              ...
              data_json TEXT NOT NULL DEFAULT '{}', UNIQUE(publication_id, window)
            );
            """)
    ...
    def register(self, identity: dict) -> dict:
        ...
        with self._connect() as conn:
            ...
            for window, delta in WINDOWS.items():
                conn.execute("INSERT OR IGNORE INTO metric_windows(publication_id,window,collect_at) VALUES(?,?,?)", (row["id"], window, (published + delta).isoformat()))
    ...
    def due_windows(self, now: datetime | None = None) -> list[dict]:
        ...
        with self._connect() as conn:
            rows = conn.execute("SELECT * FROM metric_windows WHERE status='pending' AND collect_at<=?", (now.isoformat(),)).fetchall()
    ...
    def record_metrics(self, platform: str, account_alias: str, content_id: str, window: str, metrics: dict) -> bool:
        ...
        with self._connect() as conn:
            ...
            if not row:
                return False
            conn.execute("UPDATE metric_windows SET status='insufficient' WHERE id=?", (row["id"],)) if any(value == "insufficient" for value in safe.values()) else conn.execute("UPDATE metric_windows SET status='collected' WHERE id=?", (row["id"],))
            conn.execute("UPDATE metric_windows SET data_json=? WHERE id=?", (json.dumps(safe, ensure_ascii=False), row["id"]))
        return True
...
def identity_from_delivery(platform: str, result, *, account_alias: str | None = None, published_at: str | None = None) -> dict | None:
    ...
    return {
        "platform": platform,
        "internal_account_alias": account_alias or platform,
